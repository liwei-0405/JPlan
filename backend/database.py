...
def _parse_schedule_payload(payload: Any, user_id: str = "", date: str = "") -> Dict[str, Any]:
    ...
    default_envelope = {
        ...
        "preview_id": None,
        "preview_base_version": None,
        "preview_status": None,
        "preview_reason": None,
        "preview_schedule": None,
        "failed_repair_attempt": None,
    ...
    if isinstance(payload, dict) and payload.get("schema_version", 0) >= 3:
        envelope = {
            ...
            "travel_intent": bool(payload.get("travel_intent") or (payload.get("preferences") or {}).get("travel_intent", False)),
            "version": payload.get("version", 1),
            "schedule_constraints": payload.get("schedule_constraints") or (payload.get("preferences") or {}).get("schedule_constraints") or {},
            ...
            "optional_skipped": payload.get("optional_skipped") or [],
            "blocked_activities": payload.get("blocked_activities") or [],
            ...
            "preview_base_version": payload.get("preview_base_version"),
            "preview_status": payload.get("preview_status"),
            "preview_reason": payload.get("preview_reason"),
            "preview_schedule": payload.get("preview_schedule"),
            "failed_repair_attempt": payload.get("failed_repair_attempt"),
            "needs_reschedule": bool(payload.get("needs_reschedule", False)),
            "reschedule_reason": payload.get("reschedule_reason"),
            "needs_travel_validation": bool(payload.get("needs_travel_validation", False)),
            "last_rescheduled_at": payload.get("last_rescheduled_at"),
...
def save_plan(
    ...
    optional_skipped: Optional[List[Dict[str, Any]]] = None,
    blocked_activities: Optional[List[Dict[str, Any]]] = None,
    ...
    needs_travel_validation: bool = False,
    ...
    envelope = {
        ...
        'schedule_constraints': schedule_constraints or (preferences or {}).get("schedule_constraints") or {},
        'version': version,
        'preferences': {
            **(preferences or {}),
            'allow_clash': allow_clash,
        ...
        'optional_skipped': optional_skipped or [],
        'blocked_activities': blocked_activities or [],
        ...
        'preview_base_version': preview_base_version,
        'preview_status': preview_status,
        'preview_reason': preview_reason,
        'preview_schedule': preview_schedule,
        'failed_repair_attempt': failed_repair_attempt,
        'needs_reschedule': bool(needs_reschedule),
        'reschedule_reason': reschedule_reason,
        'needs_travel_validation': bool(needs_travel_validation),
